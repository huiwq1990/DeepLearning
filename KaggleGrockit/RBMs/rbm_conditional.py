...
class ConditionalRBM:
    ...
    def __init__(self, students, conditionalData, numVis, numHid,
                weightInit, hbInit):
        ### Set all the initial values for our parameters
        self.students = students
        self.conditionalData = conditionalData
        self.numVis = numVis
        self.numHid = numHid
        ...
        self.w_D = 0.1 * weightInit * np.random.randn(numVis, numHid)
        ...
        self.wu_D = np.zeros((numVis, numHid))
    ...
    def trainEpoch(self, epsilonW,epsilonVB,epsilonHB,
        ...
        wu_sum_D = np.zeros((self.numVis, self.numHid))

        norm = np.zeros(self.numVis)
        normCond = np.zeros(self.numVis)
        ...
        for studentID in shuffledSIDs:
            ...
            # get conditional data for student, just a list of questions he answered
            if studentID in self.conditionalData:
                cdata = np.array(self.conditionalData[studentID])
            else:
                cdata = []

            # for conditional RBM
            onesVector = np.ones(len(cdata))
            ...
            hProb_pos = self.logistic(np.dot(vSample_pos_0, s_w_vh_0) + np.dot(vSample_pos_1, s_w_vh_1) + self.w_h + np.dot(onesVector, s_w_D))
            ...
            for i in range(tSteps):
                ...
                # calculate hidden probabilities from binary visible units
                hProb_neg = self.logistic(np.dot(vSample_neg_0, s_w_vh_0) + np.dot(vSample_neg_1, s_w_vh_1) + self.w_h + np.dot(onesVector, s_w_D))
            ...
            # update wu_sum
            wu_sum_vh_0[sdata[0]] += np.outer(vSample_pos_0, hProb_pos) - np.outer(vSample_neg_0, hSample_neg)
            wu_sum_vh_1[sdata[0]] += np.outer(vSample_pos_1, hProb_pos) - np.outer(vSample_neg_1, hSample_neg)
            ...
            wu_sum_D[cdata] += np.outer(onesVector, hProb_pos - hSample_neg)
            ...
            if ((batchCounter+1) % batchSize == 0) or ((batchCounter + 1) == numStudents):
                ...
                self.wu_D = self.wu_D * momentum + ((normCond * wu_sum_D.T).T)
                ...
                self.w_D += (epsilonW * 0.1) * self.wu_D
    ...
    def calculatePrediction(self, testSet):
        ...
        for (cor, sID, qID) in testSet:
            ...
            if sID in self.conditionalData:
                cdata = np.array(self.conditionalData[sID])
            else:
                cdata = []
            onesVector = np.ones(len(cdata))
            ...
            hProb = self.logistic(np.dot(sData[1], s_w_vh_0) + np.dot(1 - sData[1], s_w_vh_1) + self.w_h + np.dot(onesVector, s_w_D))
