...
class FactoredBinomialRBM:
    ...
    def trainEpoch(self, epsilonW,epsilonVB,epsilonHB,
        ...
        for studentID in shuffledSIDs:
            ...
            # update wu_sum
            wu_sum_A_0[sdata[0][_first]] += np.outer(vSample_pos_0[_first], np.dot(self.w_B, hProb_pos)) - np.outer(vSample_neg_0[_first], np.dot(self.w_B, hSample_neg))
            wu_sum_A_0[sdata[0][_secnd]] += np.outer(vSample_pos_0[_secnd], np.dot(self.w_B, hProb_pos)) - np.outer(vSample_neg_0[_secnd], np.dot(self.w_B, hSample_neg))
            wu_sum_A_1[sdata[0][_first]] += np.outer(vSample_pos_1[_first], np.dot(self.w_B, hProb_pos)) - np.outer(vSample_neg_1[_first], np.dot(self.w_B, hSample_neg))
            wu_sum_A_1[sdata[0][_secnd]] += np.outer(vSample_pos_1[_secnd], np.dot(self.w_B, hProb_pos)) - np.outer(vSample_neg_1[_secnd], np.dot(self.w_B, hSample_neg))
            ...
            if ((batchCounter+1) % batchSize == 0) or ((batchCounter + 1) == numStudents):
                ...
                self.w_B += epsilonW * (self.wu_B - weightCost * self.w_B)
